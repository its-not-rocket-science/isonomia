...
**External construct validity.** Cross-validation against two independent datasets confirms that the EDR and SAP composites measure the constructs they claim to measure. Against V-Dem v16 (Coppedge et al. 2026), 18 isonomia systems with modern-state equivalents show EDR correlating with the liberal democracy index at r = 0.869 (p < 0.001), the egalitarian democracy index at r = 0.896 (p < 0.001), and the deliberative democracy index at r = 0.903 (p < 0.001). Individual components validate against their closest V-Dem analogues: E against freedom of movement (r = 0.837), R against civil society participation (r = 0.748), and D against judicial compliance (r = 0.671). Against Polity5 (Marshall and Gurr 2020), 13 matched systems show EDR correlating with DEMOC at r = 0.839 (p < 0.001) and SAP correlating with AUTOC at r = 0.860 (p < 0.001). The P dimension shows weaker correlation with Polity PARCOMP (r = 0.592), consistent with the expectation that P captures prestige competition more broadly than electoral participation alone. Against Seshat: Global History Databank (Equinox-2020; seshat-db.com/downloads_page/), 14 pre-modern systems spanning 3,000 BCE to 1,600 CE show the A dimension correlating with Seshat's administrative complexity composite at r = 0.604 (p = 0.022), I with the writing composite at r = 0.672 (p = 0.008), S with the military composite at r = 0.798 (p = 0.002), and the SAP composite with Seshat's admin proxy at r = 0.710 (p = 0.004). D does not correlate with Seshat's legal composite (r = 0.066, ns), consistent with the observation that Seshat measures social complexity rather than the Graeber-Wengrow freedoms: no Seshat variable captures exit, disobedience, or arrangement freedom directly. The D non-result confirms that the EDR composite is measuring something outside social complexity science's current scope. Cross-validation scripts and all matched comparison datasets are available at [URL removed for blind review].
...
**Paper 4: Cross-Validation and Predictive Testing.** Preliminary cross-validation of the EDR composite against V-Dem v16 (r = 0.869–0.903, n = 18) and Polity5 (r = 0.839, n = 13) is reported in Paper 1 (Section 10.2). Paper 4 extends this to the full V-Dem country-year dataset (n ≈ 180 countries, 1789–present), deepens the Seshat pre-modern validation (Seshat Equinox-2020, seshat-db.com/downloads_page/; preliminary results in Paper 1 Section 10.2), tests the threshold hypothesis against documented collapse events, and addresses the irreversibility question: conditions under which societies cross θ in the upward direction without external disruption.
