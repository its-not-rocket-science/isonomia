...
**The exogeneity of D₀.** Throughout this paper, D₀ (pre-existing disobedience freedom) is treated as an exogenous starting condition for a given system's entry into the lock-in process. This is a modelling convenience that papers over a real question: where does D₀ come from? An exploratory cross-sectional analysis across the 327 geocoded systems in the dataset finds that the distribution of D is itself partly structured by ecological conditions. High-latitude societies have significantly higher D than tropical ones (r = +0.316, p < 0.001, n = 327), with 84.7% of this relationship mediated through S: ecological conditions constrain state-building capacity, which sets a higher structural floor on D. Critically, the relationship is not driven by agricultural surplus accumulation — r(|latitude|, D) is non-significant for agricultural systems (r = +0.102, p = 0.45, n = 58) but significant for foraging/fishing (r = +0.386, p = 0.005, n = 52) and extraction economies — and it survives exclusion of European and North American systems (r = +0.264, p < 0.001, n = 285). The implication for the OR = 6.46 Phase 1 result is that the D₀ advantage is partly ecologically structured: systems in non-agricultural ecological settings constrained from high-S state formation begin with higher D₀ floors. This does not invalidate the succession dynamics results, which are confirmed across ecological settings, but it means the D₀ moderator captures both institutional history and ecological starting condition. Full analysis is in `src/ecology_of_freedom.py`.
...
A further dimension of the counter-case prediction concerns the ecological setting of D₀. The three highest-D₀ counter-cases — Norway (D₀ = 0.90, S = 0.15), Switzerland (D₀ = 0.90, S = 0.20), British Parliamentary System (D₀ = 0.85, S = 0.20) — all have very low S, consistent with the cross-sectional ecology finding that non-agricultural, higher-latitude ecological conditions structurally constrain sovereign capacity (see Paper 2 §8.1 and `src/ecology_of_freedom.py`). Whether the counter-cases maintained high D₀ despite high L because of deliberate institutional design, or because ecological conditions constrained the S that would otherwise have suppressed D₀, is not separable from the current data. The most plausible answer is both: ecological constraints provided the initial D₀ floor, and institutional design subsequently embedded that floor in constitutional structures robust enough to survive rising L. The sequence is then: ecological constraint → high D₀ floor → constitutional embedding of D → survival of elective basin under rising L. This is a testable sequence using altitude variation within latitudes as an instrument for ecological constraint independent of cultural tradition.
