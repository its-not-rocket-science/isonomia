...
### 7.4 Quadrant transition probabilities and the LCA of high-freedom archetypes

The trajectory dataset provides an additional empirical test of the attractor structure described in this paper: do systems actually behave as the CTMC predicts, moving toward appointment at high L and resisting movement back toward counter-hegemonic states? Empirical step-transition probabilities computed from 151 consecutive observation pairs in the 30-system trajectory dataset confirm the asymmetric attractor structure directly.

Translating each observation into one of four S-D quadrants — Counter-hegemonic (D ≥ 0.45, S < 0.45), Transitional (D ≥ 0.45, S ≥ 0.45), Locked-in (D < 0.45, S ≥ 0.45), and Fragile (D < 0.45, S < 0.45) — yields the following empirical transition probabilities: P(LI → LI) = 0.989, P(CH → CH) = 0.944, P(LI → CH) = 0.000, P(TR → LI) = 0.214. These probabilities are from observed trajectory steps, not model estimates.

The P(LI → CH) = 0.000 result — no observed direct escape from locked-in to counter-hegemonic in any of the 151 trajectory steps — is the strongest confirmation available of the one-way-valve finding from the succession transition analysis (Section 5.5). The locked-in quadrant is not merely a stable attractor: it is an empirically absorbing state at the trajectory level. The P(TR → LI) = 0.214 result shows that transitional systems face 21% per-step probability of moving toward locked-in, with zero observed movements from transitional to counter-hegemonic. The counter-hegemonic quadrant, while highly stable (94.4% retention), can lose ground to the fragile quadrant (5.6%), consistent with the survival analysis finding that 12.5% of counter-hegemonic trajectory systems eventually cross θ.

A complementary cross-sectional analysis (`src/a_decomposition.py`) establishes that R predicts D independently of S: partial r(R, D | S) = 0.415, p < 0.001, surviving full control at 0.283. Dynamic validation from the trajectory survival data gives C(R) = 0.879, log-rank p = 0.023 for an R median split. The G&W-discussed systems are significantly distinguished from the rest by R, E, D, and S (all p < 0.05), but not by P (p = 0.32) — their freedom is structural (exit and reorganisation), not institutional-competitive. This directly supports the Paper 3 finding that consensus succession requires both high D and high R: the quantitative evidence confirms that the right to reorganise is a structural resource for disobedience independent of sovereign capacity, and that the systems Graeber and Wengrow identify as free achieved that freedom through the same structural pathway that the CTMC identifies as the precondition for consensus succession. Both achieve high D and low S, but through different mechanisms:

*Civic-competitive (n = 51): P = 0.65, A = 0.40, E = 0.65.* Voice-mechanism systems. Freedom maintained through institutional competition for authority (assemblies, thing-systems, guild republics). Corresponds structurally to Regime I societies that have some administrative development but constrain it through competitive politics.

*Mobile-stateless (n = 85): P = 0.50, A = 0.24, E = 0.73.* Exit-mechanism systems. Freedom maintained through resource dispersal and mobility illegibility. Corresponds to Regime I societies that resist administrative development through ecological and organisational illegibility.

The distinction is confirmed as genuine by the GMM finding it independently of any prior KMeans clustering (Mann-Whitney P, p < 0.001; A, p < 0.001 within the subspace). It maps directly onto the Regime I / Regime II framework from Section 7.3: both archetypes are Regime I systems, but they maintain their Regime I status through different structural pathways. The policy implication is different for each. Civic-competitive systems can be incorporated into Regime II through the erosion of institutional competition (P falls as electoral systems are captured or administrative appointment replaces competitive selection). Mobile-stateless systems are incorporated through sedentarisation and resource legibility (A rises as dispersed resources become capturable). The historical evidence suggests both pathways are documented in the colonisation and absorption of pre-state societies, with ecological illegibility often being the more durable defence. Full analysis is in `src/trajectory_cluster_lca.py`.
...
**Figure 5. Trajectory clustering, quadrant transition probabilities, and latent class analysis.** Generated by `src/trajectory_cluster_lca.py` from `data/d_trajectory_parsed.csv` and `data/governance_extended.csv`. **Panel A:** DTW trajectory clusters (k = 3, silhouette = 0.656). All 30 trajectory systems plotted on a normalised time axis [0, 1] so trajectories spanning different calendar durations are comparable by shape. Cluster 2 (green, n = 4, rising counter-hegemonic) contains the Paper 2 counter-cases. Cluster 3 (amber, n = 7, falling high-D) contains the failed counter-hegemonic systems starting above θ. Cluster 1 (slate, n = 19, descending/stable) contains all hegemonic-mechanism systems. **Panel B:** Empirical quadrant transition probabilities from 151 trajectory step-pairs. Cells show row-normalised probabilities. P(LI → LI) = 0.989; P(LI → CH) = 0.000; P(TR → LI) = 0.214. Confirms asymmetric attractor structure. **Panel C:** LCA in the high-D/low-S subspace (D ≥ 0.65, S ≤ 0.35, n = 136) plotted on P versus E axes. Orange = Mobile-stateless (exit-based, n = 85); green = Civic-competitive (voice-based, n = 51). Diamonds indicate GMM class means. Background grey points are all other systems for context. MW P: p < 0.001; MW A: p < 0.001.
