...
> **Abstract**
>
> The lock-in sequence described in Paper 2 of this series predicts that rising surplus legibility (L) should shift the stationary distribution of succession mechanisms toward bureaucratic appointment and hereditary forms as exit freedom (E) and disobedience freedom (D) are suppressed. We test this prediction using a three-phase empirical analysis of succession mechanism change across 389 governance systems spanning 12,000 years. Phase 1 (static attraction basin analysis, n = 117 hand-coded systems) finds that L and pre-existing disobedience freedom (D₀) together strongly predict succession type at any cross-section: χ² = 25.08 (df = 4, p < 0.0001), with an odds ratio of 6.46 for D₀ in predicting elective versus hereditary succession in high-L systems. High-L elective systems have a median D₀ of 0.55 versus 0.17 for high-L hereditary systems (Mann-Whitney U = 278, p = 0.0001, rank-biserial r = −0.782). Phase 2 (empirical transition matrix, 43 type-changing events across 44 systems) shows that elite reform events produce 0% hereditary outcomes (11 cases), internal succession crises produce 56% hereditary outcomes (5 of 9 cases), and the conditional probability of transitioning to appointment succession is substantially higher at high L (40%) than at low L (17%), with this L-conditional shift confirmed by logistic regression (OR = 2.24, permutation p = 0.005). Phase 3 (continuous-time Markov chain, 149 spells, 39 events, 110,589 system-years) estimates duration-weighted transition rates and computes L-conditional stationary distributions. The stationary probability of appointment succession rises from π = 0.136 at low L to π = 0.595 at high L (+0.460), while elective succession falls from π = 0.394 to π = 0.115 (−0.279). The D₀ moderator prediction is confirmed dynamically: elective succession systems with high D₀ (≥0.45) exit elective succession at 0.55 per 1000 years versus 2.74 per 1000 years for low-D₀ elective systems (ratio 5.03×), meaning pre-existing disobedience freedom quintuples the stability of elective succession. The primary statistically significant result is the logistic regression on transition outcomes (L → appointment, permutation p = 0.005); the CTMC stationary distributions provide a theoretically principled dynamic formalisation of the same signal. A theoretically important secondary finding concerns consensus succession: the CTMC stationary probability of consensus is approximately equal at low and high L (π ≈ 0.04), but the observed cross-sectional frequency is 16%. This discrepancy reflects that consensus is a founding state, not an equilibrium state — pre-state and small-scale societies begin as consensus forms without transitioning from other types, and once those transitions begin, the pathway back to consensus is nearly closed. This finding directly supports the Graeber-Wengrow claim that the permanent loss of specific freedoms is qualitatively different from their temporary suppression.
...
This result must be interpreted with appropriate caution about the denominator. The 2.744 rate is based on only 2 events in 729 system-years in the low-D₀ elective group. The standard error on this estimate is large. A logrank test comparing the two Kaplan-Meier survival curves yields p = 0.116 — not significant by conventional thresholds. The result should be read as a strong directional confirmation of the static Phase 1 finding (where D₀ effects are well-powered and statistically clear) rather than as an independent high-powered test. What Phase 3 adds to Phase 1 is the dynamic mechanism: it is not only that high-D₀ systems tend to be elective at any cross-section, but that when they become elective, they *stay* elective significantly longer.
