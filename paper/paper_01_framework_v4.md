...
The SAP composite is the unweighted arithmetic mean of the three indices: **SAP = (S + A + P) / 3**. Weighting reflects the theoretical equivalence of the three domination forms as identified by Graeber and Wengrow; robustness checks with alternative weightings (S-dominant, A-dominant) are reported in Section 9.2.
...
Analysis of the full 401-system dataset (hand-coded n = 137, auto-coded n = 264) finds an EDR-SAP correlation of r = −0.725 (hand-coded subset, conf ≥ 2: r = −0.755, p < 0.001). All quantitative analyses use the hand-coded subset. This is consistent with H1: high-domination configurations systematically suppress the three freedoms. The relationship holds across all major historical periods and geographic regions.

The EDR distribution is bimodal (Figure 3). Systems cluster in two distinct regimes — a high-EDR cluster (EDR > 0.55) and a low-EDR cluster (EDR < 0.35) — with a trough at θ ≈ 0.45. The threshold value is identified empirically as the trough of the bimodal distribution using Jenks natural breaks optimisation on the hand-coded n = 125 subset: the Jenks break falls at EDR = 0.449. Gaussian mixture model fitting (two-component GMM, scikit-learn GaussianMixture) yields component means of 0.21 and 0.73 with crossover at EDR = 0.452, consistent with the Jenks result to two decimal places. We adopt θ = 0.45 as a round-number approximation of both estimates. Robustness: the main empirical findings (bimodal distribution, collapse mode asymmetry, lock-in counter-case analysis) are replicated unchanged at θ = 0.40 and θ = 0.50; the threshold is not a sensitive parameter. Forty-two percent of all systems fall below θ = 0.45. The bimodality is consistent with a genuine phase boundary rather than a smooth gradient: governance systems do not populate the threshold zone but are pushed above or below it by the dynamics described in Section 5.
...
The surplus legibility values (L) for all case studies below are derived by applying the coding scheme criteria to the primary archaeological and historical literature cited: specifically, the degree to which the economy is grain-based (grain being uniquely storable and countable), the presence of fixed settlement and land tenure, and the capacity of external agents to assess and tax productive output. L values are stated to two significant figures and carry ±0.10 uncertainty at confidence level 2; the Uruk and Inca cases have been independently reviewed against Jennings (2016) and D'Altroy (2002) respectively. The Zomia L value follows Scott (2009) Chapter 7 directly, which quantifies illegibility strategies explicitly. Full source mapping is in `data/coding_scheme.md`.
...
We construct a governance citation network from the Comparators and Comparative Insights columns of the source dataset, in which nodes are governance systems and edges encode the relationship between citing and cited system. Three edge types are distinguished: *comparator* (general citation of structural similarity), *parallel* (explicit identification of parallel governance forms in Comparative Insights), and *contrast* (explicit identification of contrasting forms). The resulting network contains 353 nodes and 773 edges (204 parallel, 120 contrast, 449 comparator), with a largest connected component of 348 nodes (89%).
...
**Contagion decay.** EDR similarity between governance systems propagates through the citation network with a characteristic decay across the hand-coded subgraph (n = 88 nodes with at least one edge): r = 0.776 at degree-1 (p < 0.001), r = 0.675 at degree-2 (p < 0.001), r = 0.535 at degree-3 (p < 0.001). All three are statistically significant, confirming that the network has meaningful EDR structure at short to medium range. A degree-4 sign reversal — consistent with a bipolar two-regime network topology — is established in Paper 4 from a historically-sourced 56-edge governance influence network (r_d4 = −0.649, p < 0.001, n = 30 systems with fully documented edges). The full citation network is underpowered for this test at the current dataset size; Paper 4 §3 provides the reproducible evidence for bipolar structure.
...
The primary dataset (`governance_extended.csv`) was constructed by merging and extending the `global-governance-models` repository (Schleiferdyne Systems, 2026) with 67 theory-driven additions targeting cases critical to the framework's empirical grounding. The final dataset contains 401 governance systems from deep prehistory to the present (core statistical analyses spanning 2,600 years), drawing on archaeological, ethnographic, and historical sources across all inhabited regions.

**Inclusion and exclusion criteria.** A governance system is included if: (1) it constitutes a sufficiently distinct and documented political unit that at least three of the ten continuous variables can be independently estimated from primary or secondary sources; (2) it spans at least one generation (approximately 25 years) in a stable configuration; and (3) it is not simply a sub-unit or administrative district of a system already coded at a higher level (avoiding double-counting). Systems with fewer than three estimable variables are excluded. Administrative subdivisions of already-coded polities (e.g. individual Egyptian nomes, individual Hanseatic member-cities) are excluded unless they exhibit sufficiently distinct governance properties to warrant separate coding, in which case both the system and the sub-unit are coded and the relationship is noted. The 67 additions to the upstream dataset were selected to ensure coverage of: (a) all governance systems discussed by Graeber and Wengrow (2021) in detail (n = 14 by the `gw_discussed` column); (b) systems representing understudied regions (sub-Saharan Africa, pre-Columbian Americas, Pacific) to address geographic bias in the upstream dataset; and (c) systems at theoretically important positions in the phase space (high-L/high-EDR counter-cases; systems near the θ boundary).
...
Three confidence levels are assigned to each system by the primary coder:

- **Confidence 3**: Direct primary evidence — archaeological reports, administrative records, legal codes, or contemporary firsthand accounts — for the specific variables coded. The coder can cite a specific source passage for each variable estimate. Examples: Egyptian Old Kingdom (royal annals, pyramid records, Wilkinson 1999 archaeological survey), Tokugawa Shogunate (Bakufu administrative records, sankin-kōtai documentation, Totman 1967), Confucian Bureaucracy (Tang huiyao examination records, census registers, Twitchett 1979).
- **Confidence 2**: Inference from well-documented structural features of the system, supported by named secondary sources. The variable estimate is a reasoned inference from documented institutional properties rather than a direct measurement. Examples: Gadaa System (ethnographic documentation of age-set rotation cycle, Legesse 1973), Iroquois Confederacy (recorded Great Law of Peace, Morgan 1851; clan mothers' documented veto authority, Mann 2000).
- **Confidence 1**: Auto-coded from governance type and regional/temporal characteristics using a heuristic model derived from the hand-coded subset. These entries carry a "requires human review" flag and are not used in any quantitative analysis in this paper.

Coding was performed by the author (P. Schleifer). All 125 hand-coded systems were coded against the criteria in `data/coding_scheme.md`. A blind re-coding of 25 systems by the same coder after a six-week gap (intra-rater reliability) yielded MAD = 0.072 on SAP composites and MAD = 0.089 on EDR composites. Full inter-rater reliability results are reported in §9.7. The coding confidence scale and anchor cases are fully documented in `data/coding_scheme.md`, which is included in the public repository.

Of 401 systems, 125 are hand-coded at confidence 2 or 3. All quantitative findings reported in this paper use the hand-coded subset unless otherwise noted. Auto-coded systems (n = 264, confidence 1) are included in network visualisations only, and this is noted where they appear.

### 8.3 Network construction

The governance citation network was constructed from the Comparators and Comparative Insights columns of the source dataset, using a canonical name resolution algorithm to consolidate name variants and deduplicate entries (see `src/schismogenesis.py` for full implementation). The resulting network contains 353 nodes and 773 edges. Edge types (comparator, parallel, contrast) were derived from the syntactic structure of Comparative Insights entries (keywords "Parallels:" and "Contrasts:") supplemented by the Comparators column.
...
### 8.4 Convergent validity: system-level cross-validation

The component-level validation reported in the abstract uses purpose-matched sub-indices (E vs movement freedom, R vs civil society, D vs judicial compliance) for the subset of systems with V-Dem and Polity5 coverage. A complementary system-level validation uses the composite EDR score against headline indices, enabling reproduction and transparent audit. Using `src/validate_external.py` and a crosswalk of 15 matched modern governance systems (`data/external_crosswalk.csv`):

- **V-Dem v2x_libdem** (Liberal Democracy Index, v16): composite EDR r = 0.840, Spearman r = 0.871 (p < 0.001, n = 15)
- **Polity5 polity2** (rescaled 0–1): composite EDR r = 0.817, Spearman r = 0.874 (p < 0.001, n = 15)

**Table 1. Convergent validity of Isonomia variables against external datasets.**
Each row specifies the Isonomia-side variable and the external variable to which it is compared.
SAP composite = (S + A + P) / 3; EDR composite = (E + D + R) / 3.

| Isonomia variable | External dataset | External variable | r | n | p |
|---|---|---|---|---|---|
| EDR composite | V-Dem v16 | v2x_libdem (Liberal Democracy Index) | 0.868 | 16 | < 0.001 |
| EDR composite | V-Dem v16 | v2x_egaldem (Egalitarian Democracy Index) | 0.899 | 16 | < 0.001 |
| EDR composite | V-Dem v16 | v2x_delibdem (Deliberative Democracy Index) | 0.912 | 16 | < 0.001 |
| E (exit freedom) | V-Dem v16 | v2x_freemov (freedom of movement) | 0.893 | 16 | < 0.001 |
| R (arrangement freedom) | V-Dem v16 | v2x_cspart (civil society participation) | 0.776 | 16 | < 0.001 |
| D (disobedience freedom) | V-Dem v16 | v2jucomp (judicial compliance) | 0.650 | 16 | 0.006 |
| EDR composite | V-Dem v16 (system-level reproduced) | v2x_libdem | 0.840 | 15 | < 0.001 |
| EDR composite | Polity5 | DEMOC (rescaled 0–1) | 0.847 | 11 | 0.001 |
| SAP composite | Polity5 | AUTOC (rescaled 0–1) | 0.895 | 11 | < 0.001 |
| S (sovereignty) | Polity5 | AUTOC | 0.870 | 11 | 0.001 |
| D (disobedience freedom) | Polity5 | XCONST (executive constraints) | 0.747 | 11 | 0.008 |
| EDR composite | Polity5 (system-level reproduced) | polity2 (rescaled 0–1) | 0.817 | 15 | < 0.001 |
| E (exit freedom) | WJP Rule of Law Index 2025 | Sub-factors 4.4, 4.6 (expression, privacy) | 0.904 | 7 | 0.005 |
| R (arrangement freedom) | WJP Rule of Law Index 2025 | Sub-factors 4.7, 3.3 (assembly, civic participation) | 0.861 | 7 | 0.013 |
| EDR composite | Freedom House FIW 2013–2021 | Civil Liberties aggregate | 0.923 | 9 | < 0.001 |
| E (exit freedom) | Freedom House FIW 2013–2021 | Expression and Belief (D sub-score) | 0.895 | 9 | 0.001 |
| R (arrangement freedom) | Freedom House FIW 2013–2021 | Associational Rights (E sub-score) | 0.943 | 9 | < 0.001 |
| D (disobedience freedom) | Freedom House FIW 2013–2021 | Rule of Law (F sub-score) | 0.866 | 9 | 0.003 |
| D (disobedience freedom) | CCP Comparative Constitutions v5 | Constitutional constraint composite | 0.681 | 11 | 0.021 |
| SAP composite | Seshat Equinox-2020 | Social complexity indices | 0.604–0.774 | 14 | < 0.05 |

*Note: V-Dem system-level reproduced values (n = 15) are from `src/validate_external.py` using `data/external_crosswalk.csv`; see §8.4 and the repository for full details. The n = 16 V-Dem and n = 11 Polity5 rows are component-level correlations using purpose-matched sub-indices.*

Three system-level outliers are theoretically interpretable and document productive disagreements between the frameworks rather than measurement failures. **Botswana Tribal Democracy** scores markedly higher on V-Dem (0.552) than on Isonomia EDR (0.270): V-Dem's liberal democracy index weights electoral institutions and freedom of expression, while the Isonomia framework weights practical exit freedom and arrangement freedom, both of which are constrained by traditional authority structures (the *kgotla* system, customary land tenure) that sit outside the electoral sphere. **Rwanda** scores low on both indices (Isonomia 0.480, V-Dem 0.119), with the gap reflecting V-Dem's stronger penalty for press freedom and judicial independence constraints that are more severe than the Isonomia D variable alone captures. **Swiss Consensus Democracy** scores higher on Isonomia EDR (0.790) than on V-Dem v2x_libdem (0.620): the cantonal sovereignty and direct-democracy structures produce high arrangement and exit freedom scores in the Isonomia framework, while V-Dem's liberal democracy index registers the historical civic exclusions of the Swiss system (women's suffrage only from 1971 at federal level; cantonal variation in civic rights) that do not affect the arrangement and exit dimensions.

These discrepancies confirm that the Isonomia framework is measuring a substantively distinct construct from liberal-democratic indices: not electoral participation but the structural capacity for exit, disobedience, and institutional reorganisation. The 0.84 and 0.82 composite correlations establish that the two frameworks converge substantially at the system level while diverging in ways that are theoretically interpretable.

### 8.5 Reproducibility

All data, code, and figures are publicly available at https://github.com/its-not-rocket-science/isonomia under MIT licence. Analysis scripts were developed with the assistance of Anthropic Claude (claude.ai); all statistical results were independently verified by the author and all dataset coding was performed by the author against the published coding scheme. The repository contains analysis scripts in `src/`: `phase_space.py` (SAP ternary and EDR visualisations), `edr_resilience.py` (resilience threshold analysis and collapse mode breakdown), `schismogenesis_abm.py` (network construction, schismogenesis ABM, and statistical tests), `geo_contagion_analysis.py` (geographic schismogenesis and contagion decay), `crossvalidate_edr.py` (component-level cross-validation against V-Dem v16, Polity5, WJP, Freedom House FIW, CCP, and Seshat), and `validate_external.py` (reproduced system-level composite EDR cross-validation against V-Dem and Polity5; requires `data/external_crosswalk.csv`). `validate_external.py` accepts: `--vdem` (V-Dem country-year CSV, available at https://www.v-dem.net), `--polity` (Polity5 XLS or CSV, available at https://www.systemicpeace.org/inscrdata.html), `--wjp` (WJP index CSV), and `--coverage` (crosswalk coverage report, requires no external data). `crossvalidate_edr.py` additionally accepts `--seshat`, `--fiw`, and `--ccp`. None of these external datasets is redistributed in the repository. All figures in this paper can be regenerated from the dataset by running these scripts in order. The coding scheme (`data/coding_scheme.md`) documents the operationalisation criteria for all variables, including the inter-rater reliability exercise results, the clarifications to the P and A variable definitions, and the complete external validity tables for all five cross-validation datasets.
...
EDR similarity propagates through the citation network with significant decay across three hops (hand-coded subgraph, n = 88 nodes): r = 0.776 (degree-1, p < 0.001), r = 0.675 (degree-2, p < 0.001), r = 0.535 (degree-3, p < 0.001). The contagion decay confirms that the network has genuine EDR structure: similar systems cite each other, and the similarity attenuates predictably with network distance. Evidence for a degree-4 sign reversal — the bipolar structure signature — is reported in Paper 4 from a 56-edge historically-sourced network with fully documented edges (r_d4 = −0.649, p < 0.001, n = 30); see Paper 4 §3.
...
**Auto-coded systems.** The auto-coded systems (n = 264) are included in network analysis and full-dataset statistics, but their EDR values are heuristic approximations derived from governance type and characteristics. The auto-coding systematically regresses toward the mean, which partially explains the lower correlation in the full dataset (r = −0.725) versus the hand-coded subset (r = −0.755, conf ≥ 2, n = 137). All substantive claims in this paper are based on the hand-coded subset.
...
**Figure 1. SAP Phase Space.** Each of 401 governance systems positioned in the continuous three-dimensional space defined by sovereignty (S), administration (A), and competitive politics (P) indices, visualised as a ternary plot. Node colour encodes EDR composite (red = low, green = high). Large nodes are hand-coded (confidence ≥ 2); small nodes are auto-coded (confidence 1). The dashed line on the colourbar marks θ = 0.45.

**Figure 2. EDR vs. SAP Scatter.** EDR composite (y-axis) plotted against SAP composite (x-axis) for 401 systems, coloured by collapse mode. Dashed line: linear trend for hand-coded subset (r = −0.755, conf ≥ 2, n = 137). Shaded region: fragile regime (EDR < θ = 0.45). Large dots: hand-coded; small dots: auto-coded.

**Figure 3. EDR Distribution.** Histogram of EDR composite values across all 401 systems (light blue) and hand-coded subset (dark blue). Dashed red line: resilience threshold θ = 0.45. Dotted blue line: overall mean (0.47). Shaded region: fragile regime.
...
**Figure 6. Contagion Decay.** Left panel: EDR correlation by network distance (hand-coded subgraph, n = 88 nodes): r = 0.776 at d=1, 0.675 at d=2, 0.535 at d=3 (all p < 0.001). Right panel: Decay by edge type. The degree-4 sign reversal is reported in Paper 4 §3 from the historically-sourced 56-edge network (r_d4 = −0.649, p < 0.001).
...
**Paper 1 (this paper): The Framework.** SAP phase space, EDR resilience composite, empirical validation across 401 systems. Establishes the theoretical scaffolding and primary empirical findings.
