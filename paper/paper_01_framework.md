...
Several limitations should be noted. First, the coding of continuous 0–1 indices from archaeological and historical sources involves interpretive judgements that cannot be fully formalised. The coding scheme establishes anchor cases and inter-rater targets, but independent validation of the hand-coded subset against a blind coder has not yet been conducted. This is a priority for subsequent work.

Second, the auto-coded systems (n = 264) are included in the network analysis and full-dataset statistics, but their EDR values are heuristic approximations derived from governance type and characteristics. The auto-coding systematically regresses toward the mean, which partially explains the lower correlation in the full dataset (r = −0.775) versus the hand-coded subset (r = −0.844). All substantive claims in this paper are based on the hand-coded subset.

Third, the network analysis is constrained by the source dataset's coverage. The citation network captures explicit comparisons made in secondary sources; it does not capture the full range of historically significant governance relationships. Systems without rich secondary literature are underrepresented.

Fourth, the threshold θ = 0.45 is identified empirically from the bimodal distribution trough and has not been cross-validated against independent collapse event data. This cross-validation is a primary goal of Paper 4 in this series.
