...
> community structure. A parameter sweep over 60 combinations × 20 replicates
> identifies best-fit parameters α = 0.20, β = 0.003, γ = 0.006 (loss = 0.553).
> The calibrated model reproduces the sign reversal at the correct position
> (r_d1 = 0.651, r_d2 = 0.567, r_d3 = 0.414, r_d4 = −0.561) and achieves
> modularity M = 0.448 (target 0.557). Three structural gaps remain — the ΔEDR
> distribution of contrast edges, comparator edges, and EDR assortativity — all
> attributable to a single cause: the model assigns edge types by EDR distance,
> while the empirical network assigns them by historically structured contact
> patterns. This is a calibration ceiling, not a model failure: a distance-based
> schismogenesis rule is sufficient to generate bipolarity but not the full
> fine-grained governance differentiation that historically specific contact
> networks produce. We validate the model against three Graeber-Wengrow case
> studies from *The Dawn of Everything*: Celtic non-urbanisation (EDR 0.70 →
> 0.802 ± 0.090, stable_high confirmed), Zomia Highland Communities (0.92 →
> 0.972 ± 0.037, stable_high confirmed), and the Iroquois Confederacy /
> Haudenosaunee (0.77 → 0.616 ± 0.080 with L-coupling onset at t = 200,
> stable_then_decline confirmed). The Haudenosaunee result couples the
> schismogenesis model to the lock-in sequence (Paper 2): colonial L-imposition
> activates EDR drift that overwhelms the contrast-based stabilisation,
> reproducing the historical arc of political marginalisation.
