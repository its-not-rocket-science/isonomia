...
### 8.3 Irreversibility and the Roman case
...
Three empirical questions remain open after this analysis. First, the timescale: the Natufian-to-Uruk sequence took approximately 4,500 years; the Qin sequence compressed the same logic into two generations. What determines the speed of lock-in? The current framework suggests that the speed is a function of both L (how fast legibility increases) and the pre-existing level of institutional D (whether there is resistance to the administrative expansion). These are testable predictions against the full dataset.

Second, the Roman and Han cases both show partial post-collapse EDR restoration that stabilises at a higher level than the preceding low-EDR configuration. This suggests that the institutional residue of the collapse period, the competing administrative capacities, the surviving D mechanisms, shapes the post-collapse configuration. If so, the isonomia framework needs a memory parameter: not just the current EDR value but the institutional history that produced it.

Third, the Zomia case shows that deliberate illegibility is a viable long-run strategy for EDR preservation under external legibility pressure. Whether analogous strategies are available for communities embedded in modern digital infrastructure, and what they would look like, is the subject of Paper 5 of this series.
