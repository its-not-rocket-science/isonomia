...
**Paper 3: Succession Mechanism Attraction Basins and the Markov Transition Model.** Analysis of how succession mechanism type (hereditary, elective, appointment, consensus, rotation) varies as a function of surplus legibility (L) and pre-existing disobedience freedom (D₀). Phase 1 (static model) finds that L and D₀ together predict succession type with chi-square p < 0.0001 and an odds ratio of 6.46 for D₀ in elective versus hereditary systems at high L (n = 117, confidence 2–3). The D₀ moderator prediction — that high pre-existing D₀ resists hereditary succession even at high L — is confirmed at the cross-sectional level and motivates the dynamic model. Phase 2 (transition matrix) documents 37 within-system succession transitions across 24 systems; elite reform events produce 0% hereditary outcomes (8/8); internal succession crises produce 100% hereditary outcomes (4/4). Phase 3 (Markov chain model, in progress) will estimate L-conditional stationary distributions over succession types, testing the prediction that high D₀ shifts the stationary distribution toward elective and rotation types even as L increases.
